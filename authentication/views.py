...
from core.models import LogAktivitas, Pegawai


def _get_pegawai(user):
    """
    Ambil object Pegawai yang terhubung ke user ini.
    Return None kalau belum ada (misal user baru belum punya profil Pegawai).
    """
    try:
        return Pegawai.objects.get(user=user)
...
class OTPSetupView(View):
    ...
    def _generate_qr_code_base64(self, uri):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user.verify_otp(token):
                ...
                # Catat log login pertama kali
                LogAktivitas.objects.create(
                    pegawai=_get_pegawai(user),
                    nama_pegawai=user.get_full_name() or user.username,
                    jenis_aksi='Login',
                    keterangan='Login pertama kali + setup OTP',
                )
...
class OTPVerifyView(View):
    ...
    def get(self, request):
        ...
        if not user:
            messages.error(request, 'Sesi tidak valid. Silakan login ulang.')
            return redirect('authentication:login')
        return self._render(request, user, OTPVerifyForm())
        ...
        if form.is_valid():
            ...
            if user.verify_otp(token):
                ...
                LogAktivitas.objects.create(
                    ...
                    nama_pegawai=user.get_full_name() or user.username,
                    jenis_aksi='Login',
                    keterangan=f'Login berhasil dari OTP verify',
                )
                return redirect(reverse(user.get_dashboard_url()))
            else:
...
class CustomLogoutView(View):
    def post(self, request):
        nama = request.user.get_full_name() or request.user.username if request.user.is_authenticated else 'Unknown'
        # Catat log logout
        try:
            LogAktivitas.objects.create(
                pegawai=_get_pegawai(request.user) if request.user.is_authenticated else None,
                nama_pegawai=nama,
                jenis_aksi='Logout',
...
class DashboardAdminView(AdminRequiredMixin, View):
    """
    Redirect ke core:dashboard_admin yang lebih lengkap
    (ada stat dokumen HRD, Staff, dan log aktivitas terbaru).
    """
    def get(self, request):
...
class UserCreateView(AdminRequiredMixin, CreateView):
    ...
    def form_invalid(self, form):
        messages.error(self.request, 'Terdapat kesalahan pada form.')
...
class UserResetOTPView(AdminRequiredMixin, View):
    def post(self, request, pk):
        ...
        messages.success(request,
            f'OTP untuk pengguna "{user.username}" berhasil direset.')
        return redirect('authentication:user_list')
